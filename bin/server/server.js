...
var keys = ['ttoken1', 'ttoken2', 'ttoken3', 'ttoken4', 'ttoken5', 'syncinussecrettoken', 'atlantissecrettoken'];
...
class Entity {
    ...
    refreshBodyAttributes() {
        ...
        this.topSpeed = c.runSpeed * (this.SPEED * 4.675) * this.skill.mob / speedReduce / 2.75;
    ...
    confinementToTheseEarthlyShackles() {
        ...
        if (room.gameMode === 'tdm' && this.type !== 'food') {
            ...
            if (this.team !== -1 && room.isIn('bas1', loc) || this.team !== -2 && room.isIn('bas2', loc) || this.team !== -3 && room.isIn('bas3', loc) || this.team !== -4 && room.isIn('bas4', loc) || this.team !== -5 && room.isIn('bas5', loc) || this.team !== -6 && room.isIn('bas6', loc)) {
                this.kill();
            }
        }
    }
                    ...
                    case '0':
                        {
                            // testbed cheat
                            if (m.length !== 0) {
                                socket.kick('no BT for you >:c');return 1;
                            }
                            // cheatingbois
                            if (player.body != null) {
                                if (socket.key === 'ttoken1' || socket.key === 'ttoken2' || socket.key === 'ttoken3' || socket.key === 'ttoken4' || socket.key === 'ttoken5' || socket.key === 'syncinussecrettoken' || socket.key === 'williamsecrettoken') {
                        ...
                        case "tdm":
                            {
                                ...
                                let census = [1, 1, 1, 1, 1, 1],
                                    scoreCensus = [1, 1, 1, 1, 1, 1];
                                players.forEach(p => {
                                    census[p.team - 1]++;
                                    if (p.body != null) {
                    ...
                    let cleanmapreader = (() => {
                        ...
                        (() => {
                            ...
                            return () => {
                                ...
                                let full = [team1map.exportall(), team2map.exportall(), team3map.exportall(), team4map.exportall(), team5map.exportall(), team6map.exportall()];
                                // The reader
                                return (team, everything = false) => {
            ...
            if (component >= diff.length - combinedRadius) {
                ...
                if (goahead) {
                    ...
                    if (nIsFirmCollide < 0) {
                        ...
                        my.accel.x += modifiers._me * force.x * my.velocity.x * 2;
                        my.accel.y += modifiers._me * force.y * my.velocity.y * 2;
                        n.accel.x -= modifiers._n * force.x * n.velocity.x * 2;
                        n.accel.y -= modifiers._n * force.y * n.velocity.y * 2;
                    }
                }
            }
        }
...
var maintainloop = (() => {
    ...
    function placeRoids() {
        ...
        for (let i = Math.ceil(rockcount * 9); i; i--) {
            count++;placeRoid('rock', Class.obstacle);
        }
        for (let i = Math.ceil(rockcount * 8.5); i; i--) {
    ...
    let makefood = (() => {
        ...
        function getFoodClass(level) {
            ...
            switch (level) {
                ...
                case 6:
                    a = Class.megaPentagon;break;
                case "h":
                    a = Class.hexagon;break;
                case "d":
                    a = Class.dodecagon;break;
                default:
                    ...
                    try {
                        ...
                        for (var key in botreturn) {
                            if (output.length > 500) {
                                console.log(output.length);bot.createMessage(msg.channel.id, output);output = '';
                            }
